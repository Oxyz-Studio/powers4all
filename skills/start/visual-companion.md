...
The browser is ALWAYS the primary interface for Powers4All. Every user interaction goes through the browser. The terminal is exclusively the agent's workspace — the user never reads or types there (beyond pressing Enter to signal they have responded in the browser).

Every question, clarification, preview, confirmation, and summary must be rendered as an HTML page in the browser.

## Design Quality Standards

Pages generated by Powers4All must be **production-grade and visually refined**. The frame template provides a polished foundation — your content should match its quality:

- **Write content fragments** — the frame template wraps them with typography, theme, and layout automatically. Do NOT write full HTML documents unless you need complete control.
- **Use the provided CSS components** — they are designed to be cohesive. Combine them creatively.
- **Create visual hierarchy** — use headings, subtitles, spacing, and sections to guide the eye. Don't dump all information at once.
- **Keep pages focused** — one purpose per page. If you have a diagnosis AND a choice, that's fine on one page. If you have a 10-step explanation, break it into multiple pages.
- **Use contextual HTML/CSS** when built-in components aren't enough — the frame provides CSS variables (`var(--accent)`, `var(--bg-secondary)`, etc.) you can use in inline styles. Create cards, visual comparisons, flow diagrams, or any layout that best communicates your content.
- **Never use generic placeholder text** — every element should contain real, relevant content.

**Available CSS variables for custom styles:**
`--bg-primary`, `--bg-secondary`, `--bg-tertiary`, `--bg-elevated`, `--border`, `--border-subtle`, `--text-primary`, `--text-secondary`, `--text-tertiary`, `--accent`, `--accent-subtle`, `--success`, `--success-subtle`, `--error`, `--error-subtle`, `--shadow-sm`, `--shadow-md`, `--shadow-lg`, `--radius-sm`, `--radius-md`, `--radius-lg`, `--radius-xl`

**Additional components beyond the basics:**
- `.alert` (`.alert-info`, `.alert-success`, `.alert-warning`, `.alert-error`) — colored info boxes
- `.badge` (`.badge-info`, `.badge-success`, `.badge-error`) — small tags/labels
- `.divider` — horizontal separator
- `.stats` + `.stat` (`.stat-value`, `.stat-label`) — metric/number cards
